...
//        dev = context->devices = new log_device_t("main", false);
        dev = context->devices = (log_device_t*)malloc(sizeof(log_device_t));
        log_device_t temp("main", false);
        memcpy(dev, &temp, sizeof(log_device_t));
        context->devCount = 1;
...
/*        if (asprintf(&buf, "%-*s", (int)(bLen - 1), setPruneList) > 0) {
    ...
    if (printStatistics || getPruneList) {
        ...
        for (int retry = 32; (retry >= 0) && ((buf = (char*)malloc(len)));
             free(buf), buf = nullptr, --retry) {
            if (getPruneList) {
